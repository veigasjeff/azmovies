...
export async function getStaticProps({ params }) {
  try {
    ...
    if (!moviesItem) {
      throw new Error(`Movie item with id ${params.id} not found`);
    }

    console.log('Fetched movie item:', moviesItem); // Logging the fetched movie item for verification

    return { props: { moviesItem } };
  } catch (error) {
    console.error('Error fetching movie item:', error.message);
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      },
      {
        ...
        author: {
          '@id': 'https://azmovies.vercel.appauthor/moviesItem/'
        },
        publisher: {
          '@id': 'https://gravatar.com/drtrailer2022/#person'
        },
