  ...
  return (
    <div>
      <Head>
        ...
        />
        <title>Watch Movie Rebel Ridge (2024) | AtoZ Movies™</title>
...
export async function getServerSideProps () {
  const res = await fetch('http://localhost:3000/moviesp12.json')
  const data = await res.json()
  const selectedMovie = data.find(movie => movie.id === 'INDEXP1202')
  return {
    props: {
      movie: selectedMovie
    }
