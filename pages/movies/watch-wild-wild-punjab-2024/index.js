...
const moviesDetail = ({ movie }) => {
  ...
  useEffect(() => {
    ...
    setLinkTargets(getRandomLinks(tvshowData, adultData, trailersData, latestData));
  ...
  const fetchData = async () => {
    try {
      ...
      const randomMoviesData = getRandomItems(data, 9)
  ...
  const enhancedParagraph = text => {
    const linkTargets = [
      { text: 'Wild Wild Punjab - 2024', url: 'https://www.imdb.com/title/tt31514262/' },
    ]
  ...
  return (
    <div>
      <Head>
        ...
        <title>Watch Movie Wild Wild Punjab (2024) | A to Z Movies™</title>
