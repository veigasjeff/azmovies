...
const getRandomLinks = (movies, tvshows, adults, trailers, count = 2) => {
...
const trailersDetail = ({ trailers }) => {
  ...
  const [currentImage, setCurrentImage] = useState(trailers.image1)

  useEffect(() => {
    const imageKeys = Object.keys(trailers).filter(key =>
      key.startsWith('image')
    )

    const updateImage = () => {
      const randomKey = imageKeys[Math.floor(Math.random() * imageKeys.length)]
      setCurrentImage(trailers[randomKey])
    }
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title>Watch The Killer Official Trailer | A to Z Trailers™</title>
        ...
        />
        <meta property='og:image' content={`${trailers && trailers.image}`} />
      ...
      >
        ...
        <div className='flex-container'>
          ...
          <div className='sidebar'>
            ...
            >
              MOST POPULAR MOVIE TRAILERS
            </p>
            <div className='categorylatest-container'>
              <div className='cardlatest-container'>
                {randomTrailers.map(trailers => (
                  <div key={trailers.id} className='cardlatest'>
                    <a href={trailers['trailers.watch']} id={trailers.id}>
