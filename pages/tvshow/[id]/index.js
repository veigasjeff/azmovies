...
export async function getStaticProps({ params }) {
  try {
    ...
    console.log('Fetched TV show item:', tvshowItem); // Logging the fetched TV show item for verification

    return { props: { tvshowItem } };
  } catch (error) {
    console.error('Error fetching TV show item:', error.message);
    return { props: { tvshowItem: null, error: error.message } };
  }
}
...
const tvshowDetail = ({ tvshowItem }) => {
  ...
  // Check if tvshowItem and videotvitem exist before accessing properties
  const isTvShow =
    tvshowItem && tvshowItem.videotvitem && tvshowItem.videotvitem.length > 0;
  ...
  const movieVideoItem =
    tvshowItem && tvshowItem.videotvshowitem && tvshowItem.videotvshowitem.length > 0
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          property='og:title'
          content={`${tvshowItem && tvshowItem.name} - A to Z Tvshow`}
        />
        <meta
        ...
        <meta property='og:url' content={`${tvshowItem && tvshowItem.siteurl}`} />
        <meta
          name='keywords'
