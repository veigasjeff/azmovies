...
const tvshowDetail = ({ tvshow }) => {
  ...
  useEffect(() => {
    ...
    setLinkTargets(getRandomLinks(moviesData, adultData, trailersData, latestData));
  ...
  const fetchData = async () => {
    try {
      ...
      const randomTvshowData = getRandomItems(data, 9);
  ...
  const enhancedParagraph = text => {
    const linkTargets = [
      { text: 'House of the Dragon Season 1 - 2024', url: 'https://www.imdb.com/title/tt11198330/' },
    ]
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title>Watch House of the Dragon Season 1 (2024) | A to Z Tvshow™</title>
